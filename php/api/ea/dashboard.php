...
if ($checks == true) {
    ...
    if ($statuss == true) {
        if ($_SESSION['active'] == true) {
            ...
            # khai báo url đồng bộ user, group, device
            $url_group = "http://$serverip/api/groups/" . $_SESSION['edge_id'];
            $url_user = "http://$serverip/api/users/" . $_SESSION['edge_id'];

            # đồng bộ
            $group = call_api("GET", $token, $url_group, false);
            $user = call_api("GET", $token, $url_user, false);
            $device = get_end_device($_SESSION["edge_id"], false);

            #check điều kiện để set $_Session
            if ($_SESSION['group'] != $group['data'] && $group['data'] != null && $group['data'] != "null") {
                unset($_SESSION['group']);
                $_SESSION['group'] = $group['data'];
            }

            if ($_SESSION['user'] != $user['data'] && $user['data'] != null && $user['data'] != "null") {
                unset($_SESSION['user']);
                $_SESSION['user'] = $user['data'];
            }

            if ($_SESSION['device'] != $device && $device != null && $device != "null") {
                                                    ...
                                                    <table class="table table-striped table-hover table-condensed">
                                                        <tbody>
                                                            <tr style="text-align: center;">
                                                                <td style="text-align: center !important;" title="<?= $test3['wan-interface']['l3_device'] ?>(<?= ($test3['wan']['macaddr'] == "") ? htmlspecialchars("Null") : htmlspecialchars($test3['wan']['macaddr']) ?>)">
                                                            ...
                                                            <tr style="text-align: center;">
                                                                <td style="text-align: center !important; title="<?= $test3['lan-interface']['l3_device'] ?>(<?= ($test3['lan']['macaddr'] == "") ? htmlspecialchars("Null") : htmlspecialchars($test3['lan']['macaddr']) ?>)">
                                                                    <i class="fa fa-sitemap"></i>
